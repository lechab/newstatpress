 ...
 */
function newstatpress_filter_for_xss( $_value ) {
	$_value = trim( $_value );
	...
	if ( 0 === strlen( $_value ) ) {
		...
		array_walk( $array_values, 'newstatpress_trim_value' );
		return $array_values;
	}
}
 ...
 */
function newstatpress_trim_value( &$value ) {
	$value = trim( $value );
}
 ...
 */
function newstatpress_print_option( $option_title, $option_var, $var ) {
 ...
 */
function newstatpress_get_schedules() {
 ...
 */
function newstatpress_interval( $since ) {
 ...
 */
function newstatpress_print_schedules( $current ) {
	global $newstatpress_option_vars;
	$schedules = newstatpress_get_schedules();
	?>
	...
	<?php foreach ( $schedules as $sched_name => $sched_data ) { ?>
		<option <?php selected( $current, $sched_name ); ?> value="<?php echo esc_attr( $sched_name ); ?>"><?php printf( '%s (%s)', esc_html( $sched_data['display'] ), esc_html( newstatpress_interval( $sched_data['interval'] ) ) ); ?></option>
	<?php } ?>
	</select>
	<?php
}
 ...
 ****************************************************/
function newstatpress_print_row_input( $option_title, $nsp_option_vars, $input_size, $input_maxlength ) {
	?>
	<tr>
	<td>
	<?php
 ...
 */
function newstatpress_input_selected( $name, $value, $default ) {
 ...
 * @param string $option_title the title for option.
 ******************************************/
function newstatpress_print_row( $option_title ) {
 ...
 */
function newstatpress_print_checked( $option_title, $option_var ) {
 ...
 */
function newstatpress_print_textaera( $option_title, $option_var, $option_description ) {
...
<div class='wrap'>
	...
	if ( ! wp_next_scheduled( 'nsp_mail_notification' ) ) {
		...
		if ( 'enabled' === $status ) {
			...
			$name            = $newstatpress_option_vars['mail_notification_time']['name'];
			$timeuser        = get_option( $name );
			$t               = time();
			$crontime_offest = newstatpress_calculation_offset_time( $t, $timeuser );
		...
		if ( 'disabled' === $status ) {
			newstatpress_mail_notification_deactivate();
			...
			if ( isset( $_POST['saveit'] ) && 'all' === $_POST['saveit'] ) {
				...
				$timeuser        = get_option( $name );
				$t               = time();
				$crontime_offest = newstarpress_calculation_offset_time( $t, $timeuser );
				$crontime        = time() + $crontime_offest;
				remove_action( 'nsp_mail_notification', 'newstatpress_stat_by_email' );
	...
	if ( isset( $_POST['saveit'] ) && 'all' === $_POST['saveit'] ) { // option update request by user.
		...
		foreach ( $newstatpress_option_vars as $var ) {
			if ( isset( $_POST[ $var['name'] ] ) ) {

				if ( 'newstatpress_ignore_ip' === $var['name'] ) {
					if ( isset( $_POST['newstatpress_ignore_ip'] ) ) {
						update_option( 'newstatpress_ignore_ip', newstatpress_filter_for_xss( sanitize_text_field( wp_unslash( $_POST['newstatpress_ignore_ip'] ) ) ) );
					}
				} elseif ( 'newstatpress_ignore_users' === $var['name'] ) {
					if ( isset( $_POST['newstatpress_ignore_users'] ) ) {
						update_option( 'newstatpress_ignore_users', newstatpress_filter_for_xss( sanitize_text_field( wp_unslash( $_POST['newstatpress_ignore_users'] ) ) ) );
					}
				} elseif ( 'newstatpress_ignore_permalink' === $var['name'] ) {
					if ( isset( $_POST['newstatpress_ignore_permalink'] ) ) {
						if ( isset( $_POST['newstatpress_ignore_permalink'] ) ) {
							update_option( 'newstatpress_ignore_permalink', newstatpress_filter_for_xss( sanitize_text_field( wp_unslash( $_POST['newstatpress_ignore_permalink'] ) ) ) );
						}
					}
		...
		$mail_confirmation = newstatpress_stat_by_email( 'test' );
	...
	<form id="myoptions" method=post>
		...
		<table class='form-tableH'>
			...
			newstatpress_print_checked( $option_title, $option_var );
			...
			newstatpress_print_option( '', $option_var, $val );
		...
		<table class='form-tableH'>
			...
			<td>
				<fieldset>
				<?php
				$name = $newstatpress_option_vars['calculation']['name'];
				$valu = $newstatpress_option_vars['calculation']['value'];
				echo "
			...
			newstatpress_print_option( $option_title, $option_var, $val );
			?>
			</tr>
			<tr>
			...
			<?php
				...
				$option_title = sprintf( __( 'Elements in Overview (default %d)', 'newstatpress' ), intval( $newstatpress_option_vars['overview']['value'] ) );
				newstatpress_print_row_input( $option_title, $newstatpress_option_vars['overview'], $input_size, $input_maxlength );
				...
				$input_size      = '10';
				$input_maxlength = '10';
				$name            = $newstatpress_option_vars['stats_offsets']['name'];
				$val             = get_option( $name );
		...
		<table class='form-tableH'>
			...
			<?php
				...
				newstatpress_print_textaera( $option_title, $option_var, $option_description );
		...
		<?php
			// translators: add int value at placeholder.
			$option_title = sprintf( __( 'Top days (default %d)', 'newstatpress' ), intval( $newstatpress_option_vars['top_days']['value'] ) );
			newstatpress_print_row_input( $option_title, $newstatpress_option_vars['top_days'], $input_size, $input_maxlength );

			// translators: add int value at placeholder.
			$option_title = sprintf( __( 'O.S. (default %d)', 'newstatpress' ), intval( $newstatpress_option_vars['os']['value'] ) );
			newstatpress_print_row_input( $option_title, $newstatpress_option_vars['os'], $input_size, $input_maxlength );

			// translators: add int value at placeholder.
			$option_title = sprintf( __( 'Browser (default %d)', 'newstatpress' ), intval( $newstatpress_option_vars['browser']['value'] ) );
			newstatpress_print_row_input( $option_title, $newstatpress_option_vars['browser'], $input_size, $input_maxlength );

			// translators: add int value at placeholder.
			$option_title = sprintf( __( 'Feed (default %d)', 'newstatpress' ), intval( $newstatpress_option_vars['feed']['value'] ) );
			newstatpress_print_row_input( $option_title, $newstatpress_option_vars['feed'], $input_size, $input_maxlength );

			// translators: add int value at placeholder.
			$option_title = sprintf( __( 'Search Engines (default %d)', 'newstatpress' ), intval( $newstatpress_option_vars['searchengine']['value'] ) );
			newstatpress_print_row_input( $option_title, $newstatpress_option_vars['searchengine'], $input_size, $input_maxlength );

			// translators: add int value at placeholder.
			$option_title = sprintf( __( 'Top Search Terms (default %d)', 'newstatpress' ), intval( $newstatpress_option_vars['search']['value'] ) );
			newstatpress_print_row_input( $option_title, $newstatpress_option_vars['search'], $input_size, $input_maxlength );

			// translators: add int value at placeholder.
			$option_title = sprintf( __( 'Top Referrer (default %d)', 'newstatpress' ), intval( $newstatpress_option_vars['referrer']['value'] ) );
			newstatpress_print_row_input( $option_title, $newstatpress_option_vars['referrer'], $input_size, $input_maxlength );

			// translators: add int value at placeholder.
			$option_title = sprintf( __( 'Countries/Languages (default %d)', 'newstatpress' ), intval( $newstatpress_option_vars['languages']['value'] ) );
			newstatpress_print_row_input( $option_title, $newstatpress_option_vars['languages'], $input_size, $input_maxlength );

			// translators: add int value at placeholder.
			$option_title = sprintf( __( 'Spiders (default %d)', 'newstatpress' ), intval( $newstatpress_option_vars['spiders']['value'] ) );
			newstatpress_print_row_input( $option_title, $newstatpress_option_vars['spiders'], $input_size, $input_maxlength );

			// translators: add int value at placeholder.
			$option_title = sprintf( __( 'Top Pages (default %d)', 'newstatpress' ), intval( $newstatpress_option_vars['pages']['value'] ) );
			newstatpress_print_row_input( $option_title, $newstatpress_option_vars['pages'], $input_size, $input_maxlength );

			// translators: add int value at placeholder.
			$option_title = sprintf( __( 'Top Days - Unique visitors (default %d)', 'newstatpress' ), intval( $newstatpress_option_vars['visitors']['value'] ) );
			newstatpress_print_row_input( $option_title, $newstatpress_option_vars['visitors'], $input_size, $input_maxlength );

			// translators: add int value at placeholder.
			$option_title = sprintf( __( 'Top Days - Pageviews (default %d)', 'newstatpress' ), intval( $newstatpress_option_vars['daypages']['value'] ) );
			newstatpress_print_row_input( $option_title, $newstatpress_option_vars['daypages'], $input_size, $input_maxlength );

			// translators: add int value at placeholder.
			$option_title = sprintf( __( 'Top IPs - Pageviews (default %d)', 'newstatpress' ), intval( $newstatpress_option_vars['ippages']['value'] ) );
			newstatpress_print_row_input( $option_title, $newstatpress_option_vars['ippages'], $input_size, $input_maxlength );
		?>
		</table>
		</div>
		...
		if ( '' === $sender ) {
			$sender = $newstatpress_option_vars['mail_notification_sender']['value'];
		}
		...
		<table class='form-tableH'>
			...
			<td>
				...
				<?php
				$name    = $newstatpress_option_vars['mail_notification']['name'];
				$default = $newstatpress_option_vars['mail_notification']['value'];
				?>
				<form id="myForm">
				<p>
					<input class="tog" type='radio' id='dis' name='<?php echo esc_attr( $name ); ?>' value='disabled'<?php newstatpress_input_selected( $name, 'disabled', $default ); ?> /><label> <?php esc_html_e( 'Disabled', 'newstatpress' ); ?></label>
				</p>
				<p>
					<input class="tog" type='radio' id='ena' name='<?php echo esc_attr( $name ); ?>' value='enabled'<?php newstatpress_input_selected( $name, 'enabled', $default ); ?>  /><label> <?php esc_html_e( 'Enabled', 'newstatpress' ); ?></label>
				</p>
				</form>
				</fieldset>
			</td>
			...
			<td>
				<?php
				$name = $newstatpress_option_vars['mail_notification_freq']['name'];
				newstatpress_print_schedules( get_option( $name ) );
				?>
			</td>
			</tr>
			<tr>
			...
			<?php
									...
									printf( esc_html__( 'UTC time is %s', 'newstatpress' ), '<code>' . esc_html( date_i18n( $time_format, false, true ) ) . '</code>' );
			?>
													</span>			
			<span id="local-time">
			<?php
						// translators: placeholders for time.
						printf( esc_html__( 'Local time is %s', 'newstatpress' ), '<code>' . esc_html( date_i18n( $time_format ) ) . '</code>' );
			?>
			</span>
